...
- On manual `useAnimatedScrollHandler` scroll listeners, set `scrollEventThrottle={16}` on the `Animated.ScrollView`/`FlatList` for smooth 60fps updates (not needed with `useScrollOffset`/`useScrollViewOffset`, which track automatically).

## Project notes (Raqm)

This project's design system (`apps/raqm/DESIGN.md`) restricts motion to a
fixed vocabulary: Appear, Fade, Reveal, Lift, Focus, Transition, Collapse.
When generating animations for this repo:

- Durations: 150–250ms for micro-interactions, 300–400ms for screen
  transitions (`withTiming`, not `withSpring`, for these).
- Easing: `Easing.out(...)` for entrances, `Easing.in(...)` for exits. No
  spring/bounce/elastic curves anywhere — this overrides the "natural bouncy
  feel" default in this skill. Do not reach for `withSpring`/`.springify()`
  unless a screen explicitly calls for a physical/press-feedback moment
  (e.g. a button scale-down on `onPressIn`), never for content entrances.
- Reveal → `entering={FadeIn...}`/`Keyframe` fade+slight-translate presets.
- Collapse → `LinearTransition` (layout transitions), not custom spring physics.
- Always wrap with `useReducedMotion()` per the OS setting (DESIGN.md §14
