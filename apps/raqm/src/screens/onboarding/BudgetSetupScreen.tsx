...
export function BudgetSetupScreen({
  navigation,
  route,
}: OnboardingScreenProps<"BudgetSetup">) {
  ...
  const [amounts, setAmounts] = useState<Record<string, string>>(
    Object.fromEntries(
      DEFAULT_CATEGORIES.map((cat) => [
        cat,
        suggested[cat] ? String(suggested[cat]) : "",
      ]),
    ),
  );
  ...
  return (
    ...
    >
      ...
      <Text className="font-instrument text-[13px] text-onb-ink-body dark:text-onb-ink-body-dark mb-[14px]">
        {isAndroid
          ? "Based on last month. Adjust anything."
      ...
      <View className="flex-1 rounded-outer border border-onb-border-subtle dark:border-onb-border-subtle-dark overflow-hidden">
              ...
              {orderedCategories.map((category) => (
                ...
                >
                  ...
                  <View className="flex-row items-center">
                    {amounts[category] && (
                      <Text className="font-mono text-[16px] text-onb-ink-headline dark:text-onb-ink-headline-dark">
                        ₹
                      </Text>
                    )}
                    <TextInput
        ...
        <View
          pointerEvents="none"
          style={{ position: "absolute", top: headerHeight, left: 0, right: 0 }}
        >
          {FADE_BAND_ALPHAS.map((alpha, i) => (
            <View
              key={i}
              style={{ height: 5, backgroundColor: withAlpha(c.bgBase, alpha) }}
            />
          ))}
        </View>
        ...
        >
          ...
          >
            <View>
              ...
              <Text className="font-instrument text-[11px] text-onb-ink-body dark:text-onb-ink-body-dark">
                Covers every category below
              </Text>
            </View>
